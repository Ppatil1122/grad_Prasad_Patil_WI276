...
public class Database {
    ...
    public static boolean updateOwner(int ownerId, String name, String username, String password, String phone,
    ...
    public static boolean updateSite(int siteNumber, String siteType, String occupancyStatus, Integer ownerId) {
        ...
        if (siteType != null && !siteType.equals("0")) {
            sql.append("site_type = ?, ");
            hasUpdate = true;
        }
        if (occupancyStatus != null && !occupancyStatus.equals("0")) {
            sql.append("occupancy_status = ?, ");
            hasUpdate = true;
        }
        ...
        try (Connection con = getConnection();
            ...
            int i = 1;
            if (siteType != null && !siteType.equals("0"))
                ps.setString(i++, siteType);
            if (occupancyStatus != null && !occupancyStatus.equals("0"))
                ps.setString(i++, occupancyStatus);
            if (ownerId != null && ownerId != -1)
                ps.setObject(i++, ownerId == 0 ? null : ownerId);
    ...
    public static boolean collectMaintenance(int siteNumber, int month, int year) {
        ...
        try (Connection con = getConnection();
            ...
            System.out
                    .println("Area: " + site.getAreaSqft() + " sqft, Rate: ₹" + rate + "/sqft, Total: ₹" + totalAmount);
            return true;
        } catch (SQLException e) {
            e.printStackTrace();
            return false;
    ...
    public static void viewOwnSites(int ownerId) {
        ...
        try (Connection con = getConnection();
            ...
            if (!hasSite)
                System.out.println("You don't own any sites yet.");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
    ...
    public static void viewPaymentHistory(int ownerId) {
        ...
        try (Connection con = getConnection();
            ...
            System.out.printf("%-10s %-10s %-12s %-12s %-12s %-25s %-10s%n", "Pay ID", "Maint ID", "Site#",
                    "Month/Year", "Paid", "Date", "Approved");
            System.out.println(
                    "----------------------------------------------------------------------------------------------------");
